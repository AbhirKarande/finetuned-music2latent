...
class Trainer:
    ...
    def train_it(self, batch):
        """
        Training iteration that handles both standard and contrastive learning.
        """
        if hparams.use_contrastive:
            # For contrastive learning, batch contains both original and transformed samples
            original = batch["original"].to(self.device)
            transformed = batch["transformed"].to(self.device)
            
            # Convert to representation
            data_original = to_representation(original)
            data_transformed = to_representation(transformed)
            data_encoder_original = to_representation_encoder(original)
            data_encoder_transformed = to_representation_encoder(transformed)
            ...
            if hparams.use_lognormal:
                ...
                w = get_sampling_weights(arbitrary_high_number, device=data_original.device)
                inds = torch.multinomial(w, data_original.shape[0], replacement=True).float()
                inds = (inds+torch.rand_like(inds))/float(arbitrary_high_number-1)
            else:
                inds = torch.rand((data_original.shape[0],), dtype=torch.float32, device=data_original.device)
                
            sigmas = get_sigma_continuous(inds)
            inds_step = get_step_continuous(inds, step)
            sigmas_step = get_sigma_continuous(inds_step)
            
            # Add noise to both original and transformed samples
            noises = torch.randn_like(data_original)
            noisy_samples_original = add_noise(data_original, noises, sigmas_step)
            noisy_samples_plus_one_original = add_noise(data_original, noises, sigmas)
            
            noises = torch.randn_like(data_transformed)
            noisy_samples_transformed = add_noise(data_transformed, noises, sigmas_step)
            noisy_samples_plus_one_transformed = add_noise(data_transformed, noises, sigmas)
            
            # Get model outputs
            with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=hparams.mixed_precision):
                if hparams.multi_gpu:
                    fdata_original, fdata_plus_one_original = self.ddp(data_encoder_original, noisy_samples_original, noisy_samples_plus_one_original, sigmas_step, sigmas)
                    fdata_transformed, fdata_plus_one_transformed = self.ddp(data_encoder_transformed, noisy_samples_transformed, noisy_samples_plus_one_transformed, sigmas_step, sigmas)
                else:
                    fdata_original, fdata_plus_one_original = self.gen(data_encoder_original, noisy_samples_original, noisy_samples_plus_one_original, sigmas_step, sigmas)
                    fdata_transformed, fdata_plus_one_transformed = self.gen(data_encoder_transformed, noisy_samples_transformed, noisy_samples_plus_one_transformed, sigmas_step, sigmas)
                
                # Calculate consistency loss
                loss_weight = get_loss_weight(sigmas, sigmas_step)
                consistency_loss = huber(fdata_original, fdata_plus_one_original, loss_weight) + \
                                 huber(fdata_transformed, fdata_plus_one_transformed, loss_weight)
                
                # Calculate contrastive loss
                # Normalize the embeddings
                fdata_original_norm = F.normalize(fdata_original, dim=1)
                fdata_transformed_norm = F.normalize(fdata_transformed, dim=1)
                
                # Compute similarity matrix
                sim_matrix = torch.matmul(fdata_original_norm, fdata_transformed_norm.t()) / hparams.contrastive_temperature
                ...
                # Combine losses
                loss = consistency_loss + hparams.contrastive_loss_weight * contrastive_loss
        else:
            # Original training logic
            data = to_representation(batch.to(self.device))
            data_encoder = to_representation_encoder(batch.to(self.device))
